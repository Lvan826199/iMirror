...
- 真机联调进行中(Windows): 已打通 设备发现→换驱动→激活(QT配置:已激活);
  当前卡点 = adapter.open 的 set_configuration 在 libusbK 下报 NOT_SUPPORTED,
  已改逐级降级(活动配置已是目标则跳过切换), 待用户 git pull 后复测 gui/record;
  全部真机 bug 与根因见 docs/已知问题与归因.md
