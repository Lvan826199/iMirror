...
- **ssiz 解析是有意与 Go 不同**：Go 版 `cmsamplebuf.go` 的 `index+i*4` 实际步长 8
  字节（疑似原版 bug，条目数 >1 时读错位），Python 按连续 u32 数组读；
  条目数为 1 时（常见情形）两者结果相同。
- Go 版 `cmformatdescription.go` 的 `extractPPS` 返回值变量命名颠倒（`pps` 实为 SPS），
  其 `.SPS/.PPS` 字段语义互换；Python 按标准 avcC 语义命名，真机联调时注意核对。
- **SPRP/TJMP/SRAT/TBAS 有意不做解析类**：Go 版解析它们只为打 DEBUG 日志
  （`messageprocessor.go:229-256`），不保存状态不使用字段；Python 记一行日志后忽略，
  行为等价。若真机联调需要看 SPRP 属性内容，再补轻量解析。
